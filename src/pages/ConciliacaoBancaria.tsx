  ...
  const handleAddEntry = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from("bank_reconciliation_entries").insert({
        company_id: companyId,
        bank_account_id: entryForm.bank_account_id,
        external_description: entryForm.external_description,
            ...
            <TabsContent value="contas" className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="hub-card-base p-5">
                  <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide block mb-1">Saldo Total</span>
                  <span className={`text-2xl font-bold ${totalBalance >= 0 ? "status-positive" : "status-danger"}`}>
                    {formatCurrency(totalBalance)}
                  </span>
                </div>
                <div className="hub-card-base p-5">
                  <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide block mb-1">Contas Ativas</span>
                  <span className="text-2xl font-bold text-foreground">{accounts?.length || 0}</span>
                </div>
                <div className="hub-card-base p-5">
                  <span className="text-xs font-medium text-muted-foreground uppercase tracking-wide block mb-1">Pendentes Conciliação</span>
                  <span className={`text-2xl font-bold ${pendingCount > 0 ? "status-warning" : "status-positive"}`}>
                    {pendingCount}
                  </span>
                </div>
              </div>
                  ...
                  <DialogContent>
                    ...
                    <form onSubmit={handleAddAccount} className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <Label>Agência</Label>
                          <Input value={accountForm.agency} onChange={(e) => setAccountForm({ ...accountForm, agency: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                          <Label>Conta</Label>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input type="number" step="0.01" value={accountForm.current_balance} onChange={(e) => setAccountForm({ ...accountForm, current_balance: e.target.value })} />
                      </div>
            ...
            <TabsContent value="conciliacao" className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <Dialog open={openEntry} onOpenChange={setOpenEntry}>
                  <DialogTrigger asChild>
                    <Button size="sm" disabled={!accounts?.length}><Plus className="w-4 h-4 mr-1" /> Importar Lançamento</Button>
                  </DialogTrigger>
                  <DialogContent>
                    ...
                    <form onSubmit={handleAddEntry} className="space-y-4">
                      <div className="space-y-2">
                        ...
                        <select
                          ...
                          onChange={(e) => setEntryForm({ ...entryForm, bank_account_id: e.target.value })}
                          required
                        >
                      ...
                      <div className="space-y-2">
                        ...
                        <Input value={entryForm.external_description} onChange={(e) => setEntryForm({ ...entryForm, external_description: e.target.value })} required />
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div className="space-y-2">
                          <Label>Valor (R$)</Label>
                          <Input type="number" step="0.01" value={entryForm.amount} onChange={(e) => setEntryForm({ ...entryForm, amount: e.target.value })} required />
                        </div>
                        <div className="space-y-2">
                          <Label>Data</Label>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <div className="hub-card-base p-4 text-center">
                  <span className="text-xs text-muted-foreground block">Conciliados</span>
                ...
                <div className="hub-card-base p-4 text-center">
                  <span className="text-xs text-muted-foreground block">Não Identificados</span>
                        ...
                        {entries.map((entry) => {
                          ...
                          return (
                            <tr key={entry.id} className="border-b border-border/50 hover:bg-muted/20">
                              ...
                              <td className={`py-2.5 px-4 text-right font-medium ${Number(entry.amount) >= 0 ? "status-positive" : "status-danger"}`}>
