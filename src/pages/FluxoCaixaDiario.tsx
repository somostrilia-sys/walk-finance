...
import { useCompanies, useBankAccounts, useFinancialTransactions } from "@/hooks/useFinancialData";
...
import {
  ...
  Search, Loader2, Eye, TrendingUp, TrendingDown, ArrowRightLeft, Download, Wallet, Plug,
} from "lucide-react";
import {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs defaultValue="recebimentos" className="w-full">
          <TabsList className="w-full justify-start mb-6 bg-card border border-border">
            ...
            <TabsTrigger value="visao-mensal" className="gap-1.5">
              <TrendingUp className="w-3.5 h-3.5" /> Visão Mensal
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="visao-mensal">
            <VisaoMensalTab companyId={companyId} />
          </TabsContent>
        </Tabs>
      </div>
    </AppLayout>
...
function VisaoMensalTab({ companyId }: { companyId?: string }) {
  ...
  const totalEntradas = txs.filter((t: any) => t.type === "entrada").reduce((s: number, t: any) => s + Number(t.amount), 0);
  const totalSaidas = txs.filter((t: any) => t.type === "saida").reduce((s: number, t: any) => s + Number(t.amount), 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader className="pb-2"><CardTitle className="text-base">Evolução Mensal (6 meses)</CardTitle></CardHeader>
        <CardContent>
      ...
      <Dialog open={bankDialogOpen} onOpenChange={setBankDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>Conectar Conta Bancária via API</DialogTitle>
          ...
          <div className="space-y-3 mt-2">
            {[
              ...
              <button key={b.name} onClick={() => toast({ title: "Em breve", description: `Integração via ${b.name} será disponibilizada em breve.` })} className="w-full flex items-center gap-3 p-4 rounded-lg border border-border hover:border-accent/40 hover:bg-muted/30 transition-all text-left">
                <div className="w-9 h-9 rounded-lg bg-muted flex items-center justify-center"><Wallet className="w-4 h-4 text-muted-foreground" /></div>
